...
LMICWrapper stores LoRaWAN messages into a deque object (STL) which acts as à FIFO list of messages waiting to be sent. Each time runloopOnce() is called, a LMIC callback job is registered to send first back object of this deque. When the callback job is performed, the first-out message is sent and removed from the deque. By this way, LoRaWAN messages are not lost if the radio or the network are not available.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4MTMwODY5MjgsLTE3MTA3MzcwNjIsNj
