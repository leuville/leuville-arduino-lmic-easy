...
LMICWrapper stores LoRaWAN messages into a deque object (STL) which acts as a FIFO list of messages waiting to be sent. Each time runloopOnce() is called (typically from main loop), a LMIC callback job is registered to send first back object of this deque. When the callback job is performed, the first-out message is sent and removed from the deque. By this way, LoRaWAN messages are not lost if the radio or the network are not available.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwNDQzNDE3NzksLTIwNjMxMDk2NjcsLT
E3MTA3MzcwNjIsNjY2MjQwOTgzLDE3NjIwMTc3MjEsLTE3Mzg3
NDczOTYsLTUzNTM2MTkwNF19
-->